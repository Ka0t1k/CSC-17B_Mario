...

{
   ...
   //Pressing the arrow keys starts the timer
    ...
    music->setMedia(QUrl("qrc:/audio/level.mp3"));
    music->play();
...
void MyScene::movePlayer(){
    ...
    */
    ...
    gameover->setPos(-(gameover->boundingRect().width() - width()) * ff, 0);
   ...
   //Third rock structure coins speed and position
    ...
    m_questbox6->setPos(1735+(m_questbox6->boundingRect().width() - width()) * ff * 5.06, m_questbox6->y());
    m_questbox7->setPos(1535+(m_questbox7->boundingRect().width() - width()) * ff * 5.06, m_questbox7->y());
    m_questbox8->setPos(1335+(m_questbox8->boundingRect().width() - width()) * ff * 5.06, m_questbox8->y());
    ...
    //Stretch speed and position
    m_stretch->setPos(2047+(m_stretch->boundingRect().width() - width()) * ff * 5.35, m_stretch->y());

    //Note Box speed and position
    m_notebox->setPos(1900+(m_notebox->boundingRect().width() - width()) * ff * 5.12, m_notebox->y());
    m_notebox2->setPos(1960+(m_notebox2->boundingRect().width() - width()) * ff * 5.12, m_notebox2->y());
    m_notebox3->setPos(2020+(m_notebox3->boundingRect().width() - width()) * ff * 5.12, m_notebox3->y());
    m_notebox4->setPos(2080+(m_notebox4->boundingRect().width() - width()) * ff * 5.12, m_notebox4->y());
    m_notebox5->setPos(2140+(m_notebox5->boundingRect().width() - width()) * ff * 5.12, m_notebox5->y());
    m_notebox6->setPos(2215+(m_notebox6->boundingRect().width() - width()) * ff * 5.12, m_notebox6->y());
    m_notebox7->setPos(2278+(m_notebox7->boundingRect().width() - width()) * ff * 5.12, m_notebox7->y());
    m_notebox8->setPos(2341+(m_notebox8->boundingRect().width() - width()) * ff * 5.12, m_notebox8->y());
    m_notebox9->setPos(2404+(m_notebox9->boundingRect().width() - width()) * ff * 5.12, m_notebox9->y());
    m_notebox10->setPos(2467+(m_notebox10->boundingRect().width() - width()) * ff * 5.12, m_notebox10->y());

    //Conveyor speed and position
    m_conveyor->setPos(4100+(m_conveyor->boundingRect().width() - width()) * ff * 5.76, m_conveyor->y());
    m_conveyor2->setPos(4290+(m_conveyor2->boundingRect().width() - width()) * ff * 5.76, m_conveyor2->y());
    m_conveyor3->setPos(4480+(m_conveyor3->boundingRect().width() - width()) * ff * 5.76, m_conveyor3->y());


    m_giantgoomba->setPos(4125+(m_giantgoomba->boundingRect().width() - width()) * ff * 5.32, m_giantgoomba->y());
    m_giantgoomba2->setPos(4300+(m_giantgoomba2->boundingRect().width() - width()) * ff * 5.32, m_giantgoomba2->y());
    m_giantgoomba3->setPos(4500+(m_giantgoomba3->boundingRect().width() - width()) * ff * 5.32, m_giantgoomba3->y());
...
void MyScene::initPlayField(){
   ...
   //Add Questionbox
    ...
    m_questbox8 = new QuestBox();
    m_questbox8->setPos(1335, m_groundLevel - m_questbox8->boundingRect().height()-150);
    addItem(m_questbox8);

    m_questbox7 = new QuestBox();
    m_questbox7->setPos(1535, m_groundLevel - m_questbox7->boundingRect().height()-150);
    addItem(m_questbox7);

    m_questbox6 = new QuestBox();
    m_questbox6->setPos(1735, m_groundLevel - m_questbox6->boundingRect().height()-150);
    addItem(m_questbox6);
    ...
    m_stretch->setPos(2047, m_groundLevel - m_stretch->boundingRect().height()-40);
    addItem(m_stretch);

    //Add Note box
    m_notebox = new NoteBox();
    m_notebox->setPos(1900, m_groundLevel - m_notebox->boundingRect().height()-150);
    addItem(m_notebox);

    m_notebox2 = new NoteBox();
    m_notebox2->setPos(1960, m_groundLevel - m_notebox2->boundingRect().height()-150);
    addItem(m_notebox2);

    m_notebox3 = new NoteBox();
    m_notebox3->setPos(2020, m_groundLevel - m_notebox3->boundingRect().height()-150);
    addItem(m_notebox3);

    m_notebox4 = new NoteBox();
    m_notebox4->setPos(2080, m_groundLevel - m_notebox4->boundingRect().height()-150);
    addItem(m_notebox4);

    m_notebox5 = new NoteBox();
    m_notebox5->setPos(2140, m_groundLevel - m_notebox5->boundingRect().height()-150);
    addItem(m_notebox5);

    m_notebox6 = new NoteBox();
    m_notebox6->setPos(2215, m_groundLevel - m_notebox6->boundingRect().height()-220);
    addItem(m_notebox6);

    m_notebox7 = new NoteBox();
    m_notebox7->setPos(2278, m_groundLevel - m_notebox7->boundingRect().height()-220);
    addItem(m_notebox7);

    m_notebox8 = new NoteBox();
    m_notebox8->setPos(2341, m_groundLevel - m_notebox8->boundingRect().height()-220);
    addItem(m_notebox8);

    m_notebox9 = new NoteBox();
    m_notebox9->setPos(2404, m_groundLevel - m_notebox9->boundingRect().height()-220);
    addItem(m_notebox9);

    m_notebox10 = new NoteBox();
    m_notebox10->setPos(2467, m_groundLevel - m_notebox10->boundingRect().height()-220);
    addItem(m_notebox10);

    //Add conveyor
    m_conveyor = new Conveyor();
    m_conveyor->setPos(4100, m_groundLevel - m_conveyor->boundingRect().height()-100);
    addItem(m_conveyor);

    m_conveyor2 = new Conveyor();
    m_conveyor2->setPos(4290, m_groundLevel - m_conveyor2->boundingRect().height()-100);
    addItem(m_conveyor2);

    m_conveyor3 = new Conveyor();
    m_conveyor3->setPos(4480, m_groundLevel - m_conveyor3->boundingRect().height()-100);
    addItem(m_conveyor3);

   //Add giant goomba
    m_giantgoomba = new GiantGoomba();
    m_giantgoomba->setPos(4125, m_groundLevel - m_giantgoomba->boundingRect().height()-150);
    addItem(m_giantgoomba);

    m_giantgoomba2 = new GiantGoomba();
    m_giantgoomba2->setPos(4300, m_groundLevel - m_giantgoomba2->boundingRect().height()-150);
    addItem(m_giantgoomba2);

    m_giantgoomba3 = new GiantGoomba();
    m_giantgoomba3->setPos(4500, m_groundLevel - m_giantgoomba3->boundingRect().height()-150);
...
void MyScene::timerEvent(QTimerEvent *){
   ...
   //m_glowbrick15->nextFrame5();
    ...
    m_stretch->nextFrame9();

    m_notebox->nextFrame10();
    m_notebox2->nextFrame10();
    m_notebox3->nextFrame10();
    m_notebox4->nextFrame10();
    m_notebox5->nextFrame10();
    m_notebox6->nextFrame10();
    m_notebox7->nextFrame10();
    m_notebox8->nextFrame10();
    m_notebox9->nextFrame10();
    m_notebox10->nextFrame10();
...
void MyScene::handleCollisionWithPlatform() {
    ...
    if(platform) {
        ...
        if (pos.x() >= glowbrickPos.x()  + platform->boundingRect().height()) {
            qDebug() << "change pos3";
...
//Goomba collision
void MyScene::checkCollidingGoomba() {
    QList<QGraphicsItem*> items = collidingItems(m_player);
    foreach (QGraphicsItem* item, items) {
