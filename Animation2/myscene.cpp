...
MyScene::MyScene(QObject *parent) :
  ...
  , m_jumpHeight(180)
  , m_fieldWidth(5500)//width of the virtual world
  , m_player()
  , m_sky(0)
  , m_Scene(0)
  , m_grass(0)
...
void MyScene::movePlayer()
{
    ...
    if (direction > 0) {
        ...
        if (m_realPos > m_fieldWidth - (width() - rightBorder)) {
            ...
            if (m_realPos - m_skippedMoving < rightBorder) {   //If the position of the character in "view coordinates", calculated by m_realPos – m_skippedMoving,
                m_player->moveBy(dx, 0);                      //is smaller than rightBorder, then move the elephant by calling moveBy()
            } else {
    ...
    */
    ...
    m_coins->setPos(300+(m_coins->boundingRect().width() - width()) * ff*5, m_coins->y());
...
void MyScene::initPlayField()
{
    ...
    m_coins = new Coin();
    startTimer( 100 );
    m_coins->setPos(300, m_groundLevel - m_coins->boundingRect().height());
    addItem(m_coins);
